  ...
  {#if entity}
    <dd>
      ...
      {#if entity.getConstraints().length > 0}
        <div>
          ...
          <ul class="constraint-list">
            {#each entity.getConstraints() as constraint (constraint.getIri())}
      ...
      {#if entity.getComponentCount() > 0}
        {#each Object.entries( entity.getComponents() ) as [key, components] (key)}
          <dl>
            <dt class="subheading">{key}</dt>
            {#each components as component (component.getIri())}
